...
class CharRNN(object):
  ...
  def __init__(self, is_training, batch_size, num_unrollings, vocab_size, 
    ...
    with tf.name_scope('embedding_layer'):
      ...
      print('embedding layer size, sequence size:',inputs) 
    ...
    #print("sliced input", sliced_inputs)  
    ...
    print('size of flatten outputs', flat_outputs.shape)
  ...
  def run_epoch(self, session, data_size, batch_generator, is_training,
    ...
    for step in range(epoch_size // divide_by_n):
      ...
      if (verbose > 0) and ((step+1) % freq == 0):
        logging.info("%.1f%%, step:%d, Average loss: %.3f, speed: %.0f words",
                     (step + 1) * 1.0 / epoch_size * 100, step, average_loss,
                     (step + 1) * self.batch_size * self.num_unrollings /
                     (time.time() - start_time))

    logging.info("Average loss: %.3f",
                 average_loss)
    return average_loss, summary_str, global_step, average_loss
