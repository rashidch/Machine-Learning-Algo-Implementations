...
def main():
    ...
    parser.add_argument('--data_file', type=str, default='data/shakespeare_train.txt', help='data file')

    parser.add_argument('--encoding', type=str, default='utf-8', help='the encoding of the data file.')
    ...
    parser.add_argument('--model', type=str, default='lstm',
                        help='which model to use (rnn or lstm ).')
    ...
    parser.add_argument('--learning_rate', type=float, default=2e-2s,
    ...
    logging.info('Creating train, valid split')
        ...
        with tf.Session(graph=graph) as session:
            ...
            for i in range(args.num_epochs):
                for j in range(args.n_save):
                    ...
                    # training step
                    ppl, train_summary_str, global_step, average_loss = train_model.run_epoch(
                        session,
                        train_size,
                        train_batches,
                        is_training=True,
                    ...
                    #print('Average Loss is appended:', average_loss)
                    # record the summary
                    ...
                    # run validation
                    valid_ppl, valid_summary_str, _,_ = valid_model.run_epoch(
                        session,
                        valid_size,
                        valid_batches, 
                        is_training=False,
                    ...
                    #put break point and sample the sequence
                    if i % 5 == 0 and i!=0:
                        print('Break Point Execution at epoch number:',i)
                        start_text = train_text[i+10:args.num_epochs+10]
                        print('The input sample text from training text:\n%s'% start_text )
                        sample = test_model.sample_seq(session, 20, start_text,
                                            vocab_index_dict, index_vocab_dict, 1, max_prob=False)
